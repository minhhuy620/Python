...
from app.schemas import SystemUser, TokenDataUser
import calendar
...
def decode_access_token(db, token) -> SystemUser:
    ...
    if datetime.utcfromtimestamp(calendar.timegm(token_data.expires.timetuple())) < datetime.utcnow():
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
